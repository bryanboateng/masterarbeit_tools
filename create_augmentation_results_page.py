...
config), keeps each method's best config (chosen once by the validation
condition), and renders one table per evaluation condition. Columns are split
into two families -- BC and DP+GPI -- separated by a gap, and inside each family
the best cell of a row is shown bold. Grippy conditions use a blue scale,
slippery ones an orange scale. Each TACIL cell also carries the realised
synthetic trajectory length.
...
# Column order fixed here; the two families are drawn with a gap between them.
METHODS = (
...
# Grippy stays blue, slippery turns orange, so a floor is told apart at a glance.
...
def _collect(*, runs: list) -> tuple[list[int], dict[tuple[int, str], BestConfig]]:
    ...
    groups: dict = {}
    selection_mean: dict[tuple[int, str, tuple], float] = {}
...
def _render_row(
    *,
    ...
    # The column that wins its family on this condition is shown bold.
...
body {
  margin: 0 auto; padding: 2rem 1.5rem 3rem; max-width: 66rem;
...
thead th, tbody th {
  ...
  padding: 0.25rem 0.4rem; font-weight: 600; white-space: nowrap;
}
th.family { font-size: 0.86rem; }
td { height: 3rem; }
td.reward { background: var(--fill); color: var(--ink); }
td a {
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  height: 100%; line-height: 1.1; color: inherit; text-decoration: none;
}
